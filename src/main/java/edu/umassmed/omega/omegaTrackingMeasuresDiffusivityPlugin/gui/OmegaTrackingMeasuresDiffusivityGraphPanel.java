...
import edu.umassmed.omega.omegaTrackingMeasuresDiffusivityPlugin.runnable.OmegaTrackingMeasuresDiffusivityGraphProducer;

public class OmegaTrackingMeasuresDiffusivityGraphPanel extends GenericPanel {
	...
	private OmegaTrackingMeasuresDiffusivityGraphProducer graphProducer;
	
	private int lineSize, shapeSize;

	public OmegaTrackingMeasuresDiffusivityGraphPanel(final RootPaneContainer parent,
			final OmegaTrackingMeasuresDiffusivityPluginPanel pluginPanel,
	...
	private void createAndAddWidgets() {
		...
		this.yAxis_cmb.addItem(GraphLabelConstants.GRAPH_NAME_MSD);
		this.yAxis_cmb.addItem(GraphLabelConstants.GRAPH_NAME_DIFF);
		this.yAxis_cmb.addItem(GraphLabelConstants.GRAPH_NAME_MSS);
		this.yAxis_cmb.addItem(GraphLabelConstants.GRAPH_NAME_UNCERT_D);
		this.yAxis_cmb.addItem(GraphLabelConstants.GRAPH_NAME_UNCERT_SMSS);
	...
	private void addListeners() {
		this.xAxis_cmb.addActionListener(new ActionListener() {
			...
			public void actionPerformed(final ActionEvent evt) {
				OmegaTrackingMeasuresDiffusivityGraphPanel.this.handleChangeAxis();
			}
		});
		...
		this.drawGraph_btt.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(final ActionEvent e) {
				OmegaTrackingMeasuresDiffusivityGraphPanel.this.handleDrawChart();
			}
		});
		this.addComponentListener(new ComponentAdapter() {
			@Override
			public void componentResized(final ComponentEvent evt) {
				OmegaTrackingMeasuresDiffusivityGraphPanel.this.handleComponentResized();
			}
		});
	}
	...
	private void handleDrawChart() {
		...
		if (xAxisSelection.equals(GraphLabelConstants.GRAPH_LAB_X_TRACK)) {
	...
	private void handleDrawTracksChart() {
		final String yAxisSelection = (String) this.yAxis_cmb.getSelectedItem();
		if (yAxisSelection.equals(GraphLabelConstants.GRAPH_NAME_MSD)) {
			this.handleTracksChart(OmegaTrackingMeasuresDiffusivityGraphPanel.OPTION_TRACK_SLOPE_LOG_MSD);
		} else if (yAxisSelection.equals(GraphLabelConstants.GRAPH_NAME_MSS)) {
			this.handleTracksChart(OmegaTrackingMeasuresDiffusivityGraphPanel.OPTION_TRACK_SLOPE_MSS);
		} else if (yAxisSelection.equals(GraphLabelConstants.GRAPH_NAME_DIFF)) {
			this.handleTracksChart(OmegaTrackingMeasuresDiffusivityGraphPanel.OPTION_TRACK_D);
		} else if (yAxisSelection
				.equals(GraphLabelConstants.GRAPH_NAME_UNCERT_SMSS)) {
			this.handleTracksChart(OmegaTrackingMeasuresDiffusivityGraphPanel.OPTION_TRACK_ERROR_SMSS);
		} else if (yAxisSelection
				.equals(GraphLabelConstants.GRAPH_NAME_UNCERT_D)) {
			this.handleTracksChart(OmegaTrackingMeasuresDiffusivityGraphPanel.OPTION_TRACK_ERROR_D);
		}
	}
	...
	private void handleChangeAxis() {
		...
		final String yAxisSelection = (String) this.yAxis_cmb.getSelectedItem();
		final String xAxisSelection = (String) this.xAxis_cmb.getSelectedItem();
		final String graphSelection = (String) this.graphType_cmb
				.getSelectedItem();
		if (((this.oldYAxisSelection != null) && this.oldYAxisSelection
				...
				&& (this.oldGraphTypeSelection != null)
				&& this.oldGraphTypeSelection.equals(graphSelection))
			return;
		if (xAxisSelection.equals(GraphLabelConstants.GRAPH_LAB_X_TRACK)) {
			//
		} else {
	...
	private void handleTracksChart(final int diffusivityOption) {
		...
		final OmegaTrackingMeasuresDiffusivityGraphProducer graphProducer = new OmegaTrackingMeasuresDiffusivityGraphProducer(
				this,
				graphType,
				diffusivityOption,
				this.maxT,
	...
	private void launchGraphProducerThread(final OmegaTrackingMeasuresDiffusivityGraphProducer graphProducer) {
