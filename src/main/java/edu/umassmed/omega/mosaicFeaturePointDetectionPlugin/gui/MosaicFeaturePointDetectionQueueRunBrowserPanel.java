...
public class MosaicFeaturePointDetectionQueueRunBrowserPanel extends GenericPanel {

	private static final long serialVersionUID = -7554854467725521545L;

	private final MosaicFeaturePointDetectionPluginPanel sdPanel;
	...
	public MosaicFeaturePointDetectionQueueRunBrowserPanel(final RootPaneContainer parentContainer,
			final MosaicFeaturePointDetectionPluginPanel sptPanel) {
	...
	private void addListeners() {
		this.dataTree.addMouseListener(new MouseAdapter() {
			...
			public void mouseReleased(final MouseEvent event) {
				MosaicFeaturePointDetectionQueueRunBrowserPanel.this.handleMouseClick(event.getPoint());
			}
		});
		this.dataTree.getModel().addTreeModelListener(new TreeModelListener() {
			@Override
			public void treeNodesChanged(final TreeModelEvent event) {
				MosaicFeaturePointDetectionQueueRunBrowserPanel.this.handleTreeChanged(event);
			}
	...
	private void handleMouseClick(final Point clickP) {
		final TreePath path = MosaicFeaturePointDetectionQueueRunBrowserPanel.this.dataTree
		...
		if (element instanceof OmegaImage) {
			...
			this.sdPanel.setRemoveButtonEnabled(true);
		}
	}
	...
	public void updateTree(
			final Map<Integer, Map<OmegaImage, List<OmegaParameter>>> imagesToProcess) {
		...
		if (imagesToProcess != null) {
			for (final Integer index : imagesToProcess.keySet()) {
				for (final OmegaImage image : imagesToProcess.get(index)
						.keySet()) {
					final DefaultMutableTreeNode imageNode = new DefaultMutableTreeNode();
					s = index + ": [" + image.getElementID() + "] "
							+ image.getName();
