...
public class OmegaTrajectorySegmentationSegmenterPanel extends GenericPanel {
	
	private static final long serialVersionUID = -6876397782525067201L;
	
	private final OmegaTrajectorySegmentationPluginPanel pluginPanel;
	
	private final List<OmegaTrajectorySegmentationTrackPanel> segmentTrajectoryPanels;
	...
	public OmegaTrajectorySegmentationSegmenterPanel(final RootPaneContainer parent,
			final OmegaTrajectorySegmentationPluginPanel pluginPanel,
	...
	private void createAndAddWidgets() {
		...
		this.selectStart_btt = new JButton(OmegaTrajectorySegmentationPluginConstants.SELECT_TRACK_START);
		this.selectStart_btt.setToolTipText(OmegaTrajectorySegmentationPluginConstants.SELECT_TRACK_START_MS);
		this.selectStart_btt.setPreferredSize(btt_dim);
		this.selectStart_btt.setSize(btt_dim);
		optionsPanel.add(this.selectStart_btt);
		
		this.selectEnd_btt = new JButton(OmegaTrajectorySegmentationPluginConstants.SELECT_TRACK_END);
		this.selectEnd_btt.setToolTipText(OmegaTrajectorySegmentationPluginConstants.SELECT_TRACK_END_MS);
		this.selectEnd_btt.setPreferredSize(btt_dim);
		this.selectEnd_btt.setSize(btt_dim);
		optionsPanel.add(this.selectEnd_btt);
		
		this.selectLast_btt = new JButton(OmegaTrajectorySegmentationPluginConstants.SELECT_TRACK_LAST);
		this.selectLast_btt.setToolTipText(OmegaTrajectorySegmentationPluginConstants.SELECT_TRACK_LAST_MS);
		this.selectLast_btt.setPreferredSize(btt_dim);
		this.selectLast_btt.setSize(btt_dim);
		optionsPanel.add(this.selectLast_btt);
		
		this.reset_btt = new JButton(OmegaTrajectorySegmentationPluginConstants.SELECT_RESET);
		this.reset_btt.setToolTipText(OmegaTrajectorySegmentationPluginConstants.SELECT_RESET_MS);
		this.reset_btt.setPreferredSize(btt_dim);
		this.reset_btt.setSize(btt_dim);
		optionsPanel.add(this.reset_btt);
		
		final JLabel lbl = new JLabel(OmegaTrajectorySegmentationPluginConstants.SELECT);
		optionsPanel.add(lbl);
		
		this.segmOn_cb = new JComboBox<String>();
		this.segmOn_cb.setToolTipText(OmegaTrajectorySegmentationPluginConstants.SELECT_FIRST_MS);
		this.segmOn_cb.addItem(OmegaTrajectorySegmentationPluginConstants.SELECT_FIRST_SPOTS);
		this.segmOn_cb.addItem(OmegaTrajectorySegmentationPluginConstants.SELECT_FIRST_MOTION);
		...
		this.segment_lbl = new JLabel(OmegaTrajectorySegmentationPluginConstants.ACTUAL_SEGM
				+ OmegaTrajectorySegmentationPluginConstants.SELECT_NONE);
	...
	private void addListeners() {
		this.segmOn_cb.addActionListener(new ActionListener() {
			...
			public void actionPerformed(final ActionEvent e) {
				OmegaTrajectorySegmentationSegmenterPanel.this.handleSegmentationOptionChanged();
			}
		});
		this.mainPanel.addChangeListener(new ChangeListener() {
			@Override
			public void stateChanged(final ChangeEvent evt) {
				OmegaTrajectorySegmentationSegmenterPanel.this.handlePanelChanged();
			}
		});
		this.scaleToFit_btt.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(final ActionEvent evt) {
				OmegaTrajectorySegmentationSegmenterPanel.this.handleAutoscaleChanged(true);
			}
		});
		this.scale1on1_btt.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(final ActionEvent evt) {
				OmegaTrajectorySegmentationSegmenterPanel.this.handleAutoscaleChanged(false);
			}
		});
		this.selectStart_btt.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(final ActionEvent evt) {
				OmegaTrajectorySegmentationSegmenterPanel.this.handleSelectStart();
			}
		});
		this.selectEnd_btt.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(final ActionEvent evt) {
				OmegaTrajectorySegmentationSegmenterPanel.this.handleSelectEnd();
			}
		});
		this.selectLast_btt.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(final ActionEvent evt) {
				OmegaTrajectorySegmentationSegmenterPanel.this.handleSelectLast();
			}
		});
		this.reset_btt.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(final ActionEvent e) {
				OmegaTrajectorySegmentationSegmenterPanel.this.handleSegmentationReset();
			}
		});
	}
	
	private void handleSegmentationOptionChanged() {
		for (final OmegaTrajectorySegmentationTrackPanel trackPanel : this.segmentTrajectoryPanels) {
	...
	public void handleAutoscale() {
		for (final OmegaTrajectorySegmentationTrackPanel singleTrajPanel : this.segmentTrajectoryPanels) {
		...
		if (this.radioButton_al == null) {
			this.radioButton_al = new ActionListener() {
				...
				public void actionPerformed(final ActionEvent evt) {
					...
					OmegaTrajectorySegmentationSegmenterPanel.this.setSegmentationType(butt.getText());
				}
			};
		}
	...
	private void setSegmentationType(final String segmName) {
		if (this.currentPanelIndex == -1)
			return;
		final OmegaTrajectorySegmentationTrackPanel singleTrajPanel = this.segmentTrajectoryPanels
				.get(this.currentPanelIndex);
		// FIXME move the string in constants
		...
		if (this.segmentationEnded) {
			...
			buf.append(OmegaTrajectorySegmentationPluginConstants.ACTUAL_SEGM);
			if (this.isSegmentOnSpotsSelection()) {
				buf.append(segm);
				buf.append(OmegaTrajectorySegmentationPluginConstants.SELECT_PUNCT);
				s = buf.toString();
			} else {
				buf.append(segm);
				buf.append(OmegaTrajectorySegmentationPluginConstants.SELECT_PUNCT);
			...
			if (this.isSegmentOnSpotsSelection()) {
				...
				s = this.segment_lbl.getText();
				s = s.replace(OmegaTrajectorySegmentationPluginConstants.SELECT_FROM_UPPER, segm
						+ OmegaTrajectorySegmentationPluginConstants.SELECT_FROM_LOWER);
			}
		}
		...
		for (final OmegaTrajectorySegmentationTrackPanel panel : this.segmentTrajectoryPanels) {
	...
	public void createSegmentSingleTrajectoryPanels(
		...
		for (final OmegaTrajectory traj : segmentationResults.keySet()) {
			...
			final OmegaTrajectorySegmentationTrackPanel panel = new OmegaTrajectorySegmentationTrackPanel(
			...
			final String s = OmegaTrajectorySegmentationPluginConstants.SEGMENT_CONFIRM + traj.getName();
			// this.mainPanel.add(s, scrollPane);
			this.mainPanel.add(s, panel);
			// panel.setTrajectory(traj);
			// FIXME move the string in constants
			if ((this.segmentationName != null)
					&& (this.segmentationName != "Nothing selected")) {
	...
	public void selectStartingROI(final OmegaROI startingROI) {
		...
		if (this.isSegmentOnSpotsSelection()) {
			...
			buf.append(OmegaTrajectorySegmentationPluginConstants.SELECT_FROM_LOWER_SPACE);
		} else {
			buf.append(OmegaTrajectorySegmentationPluginConstants.SELECT_FROM_UPPER_SPACE);
		}
	...
	public void selectEndingROI(final OmegaROI endingROI) {
		final StringBuffer buf = new StringBuffer();
		buf.append(this.segment_lbl.getText().replace(OmegaTrajectorySegmentationPluginConstants.SELECT_PUNCT,
				""));
		buf.append(OmegaTrajectorySegmentationPluginConstants.SELECT_TO_LOWER_SPACE);
	...
	public boolean isSegmentOnSpotsSelection() {
		...
		if (segmOn.equals(OmegaTrajectorySegmentationPluginConstants.SELECT_FIRST_SPOTS))
			return true;
		return false;
	}

	protected List<OmegaTrajectorySegmentationTrackPanel> getTrackPanels() {
		return this.segmentTrajectoryPanels;
	}

	public void setLineWidth(final int lineWidth) {
		this.lineWidth = lineWidth;
		for (final OmegaTrajectorySegmentationTrackPanel panel : this.getTrackPanels()) {
