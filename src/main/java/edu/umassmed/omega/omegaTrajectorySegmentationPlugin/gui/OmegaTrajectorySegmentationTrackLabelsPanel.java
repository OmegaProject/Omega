...
public class OmegaTrajectorySegmentationTrackLabelsPanel extends GenericPanel {
	...
	public OmegaTrajectorySegmentationTrackLabelsPanel(final RootPaneContainer parent,
	        final OmegaPluginArchetype plugin, final int orientation,
	...
	private void paintHLabel(final Graphics2D g2D, final int coord,
		...
		g2D.drawLine(coord, OmegaTrajectorySegmentationTrackPanel.LABELS_PANEL_SIZE - 10, coord,
	...
	private void paintVLabel(final Graphics2D g2D, final int coord,
	        final double val1, final double val2) {
		g2D.drawLine(OmegaTrajectorySegmentationTrackPanel.LABELS_PANEL_SIZE - 10, coord,
		        OmegaTrajectorySegmentationTrackPanel.LABELS_PANEL_SIZE, coord);
		if (coord == 0) {
	...
	private void paintMetricsFromZero(final Graphics2D g2D) {
		int numOfLabels = this.panelSize / OmegaTrajectorySegmentationTrackPanel.SPACE_BETWEEN_LABELS;
		if (numOfLabels == 0) {
			numOfLabels = 1;
		}
		...
		// + (numOfLabels * OmegaTrajectorySegmentationTrackPanel.SPACE_BETWEEN_LABELS));
	...
	private void paintMetricFromCenter(final Graphics2D g2D) {
		...
		final int numOfLabelsBefore = spaceBefore
		        / OmegaTrajectorySegmentationTrackPanel.SPACE_BETWEEN_LABELS;
	...
	private void rescale() {
		...
		if (this.orientation == 1) {
			dim = new Dimension(OmegaTrajectorySegmentationTrackPanel.LABELS_PANEL_SIZE,
			        this.panelSize + 30);
		} else {
