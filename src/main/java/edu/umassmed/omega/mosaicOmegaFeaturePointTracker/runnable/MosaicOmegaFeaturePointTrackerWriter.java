...
public class MosaicOmegaFeaturePointTrackerWriter implements MosaicOmegaFeaturePointTrackerRunnable {
	...
	public MosaicOmegaFeaturePointTrackerWriter(final OmegaMessageDisplayerPanelInterface displayerPanel) {
	...
	@Override
	public void run() {
		this.updateStatusSync(MosaicOmegaFeaturePointTrackerWriter.RUNNER + " started.", false);
		if (this.isKilled)
			return;
		try {
			this.trackList = MosaicOmegaFeaturePointTrackerDLLInvoker.callWriteResults();
		...
		try {
			MosaicOmegaFeaturePointTrackerDLLInvoker.callDisposeRunner();
		} catch (final Exception ex) {
			OmegaLogFileManager.handleUncaughtException(ex, true);
		}

		this.updateStatusAsync(MosaicOmegaFeaturePointTrackerWriter.RUNNER + " ended.", true);
	...
	private void updateStatusSync(final String msg, final boolean ended) {
		try {
			SwingUtilities.invokeAndWait(new Runnable() {
				@Override
				public void run() {
					MosaicOmegaFeaturePointTrackerWriter.this.displayerPanel
					.updateMessageStatus(new MosaicOmegaFeaturePointTrackerMessageEvent(msg,
							MosaicOmegaFeaturePointTrackerWriter.this, ended));
				}
			});
