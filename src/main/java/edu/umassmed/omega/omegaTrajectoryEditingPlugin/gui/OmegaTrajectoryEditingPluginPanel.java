...
import edu.umassmed.omega.omegaTrajectoryEditingPlugin.actions.OmegaTrajectoryEditingAction;

public class OmegaTrajectoryEditingPluginPanel extends GenericPluginPanel implements
	...
	private OmegaTrajectoryEditingEditorPanel trPanel;
	...
	private final Map<OmegaAnalysisRun, List<OmegaTrajectoryEditingAction>> actions,
			cancelledActions;
	...
	public OmegaTrajectoryEditingPluginPanel(final RootPaneContainer parent,
	...
	private void createAndAddWidgets() {
		...
		this.trPanel = new OmegaTrajectoryEditingEditorPanel(this.getParentContainer(), this,
				this.gateway);
		this.tabbedPane.add(OmegaTrajectoryEditingPluginConstants.EDITOR_TABNAME, this.trPanel);

		this.resPanel = new GenericTrackingResultsPanel(
				this.getParentContainer());
		this.tabbedPane.add(OmegaTrajectoryEditingPluginConstants.RESULTS_TABNAME, this.resPanel);
	...
	private void addListeners() {
		...
		this.images_cmb.addActionListener(new ActionListener() {
			...
			public void actionPerformed(final ActionEvent e) {
				OmegaTrajectoryEditingPluginPanel.this.selectImage();
			}
		});
		this.particles_cmb.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(final ActionEvent evt) {
				OmegaTrajectoryEditingPluginPanel.this.selectParticleDetectionRun();
			}
		});
		this.trajectories_cmb.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(final ActionEvent evt) {
				OmegaTrajectoryEditingPluginPanel.this.selectParticleLinkingRun();
			}
		});
		this.trajectoriesRelinking_cmb.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(final ActionEvent e) {
				OmegaTrajectoryEditingPluginPanel.this.selectTrajectoriesRelinkingRun();
			}
		});
		...
		this.hideDataSelection_mItm.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(final ActionEvent e) {
				OmegaTrajectoryEditingPluginPanel.this.handleHideDataSelection();
			}
		});
	}
	...
	private void activateNeededButton() {
		final Map<OmegaAnalysisRun, List<OmegaTrajectoryEditingAction>> actions = this.actions;
		final Map<OmegaAnalysisRun, List<OmegaTrajectoryEditingAction>> cancelledActions = this.cancelledActions;
	...
	private ActionListener getSaveNewAllActionsActionListener() {
		if (this.save_al == null) {
			this.save_al = new ActionListener() {
				@Override
				public void actionPerformed(final ActionEvent e) {
					OmegaTrajectoryEditingPluginPanel.this.saveAllActions();
				}
			};
		}
		return this.save_al;
	...
	private ActionListener getUndoLastActionActionListener() {
		if (this.undo_al == null) {
			this.undo_al = new ActionListener() {
				@Override
				public void actionPerformed(final ActionEvent e) {
					OmegaTrajectoryEditingPluginPanel.this.undoLastAction();
				}
			};
		}
		return this.undo_al;
	...
	private ActionListener getRedoLastActionActionListener() {
		if (this.redo_al == null) {
			this.redo_al = new ActionListener() {
				@Override
				public void actionPerformed(final ActionEvent e) {
					OmegaTrajectoryEditingPluginPanel.this.redoLastAction();
				}
			};
		}
		return this.redo_al;
	...
	private ActionListener getCancelAllActionsActionListener() {
		if (this.cancel_al == null) {
			this.cancel_al = new ActionListener() {
				@Override
				public void actionPerformed(final ActionEvent e) {
					OmegaTrajectoryEditingPluginPanel.this.cancelAllActions();
				}
			};
		}
	...
	private void saveAllActions() {
		...
		buf.append(OmegaTrajectoryEditingPluginConstants.SAVE_CONFIRM_MSG);

		final GenericConfirmationDialog dialog = new GenericConfirmationDialog(
				this.getParentContainer(), OmegaTrajectoryEditingPluginConstants.SAVE_CONFIRM,
				buf.toString(), true);
	...
	private void undoLastAction() {
		...
		final List<OmegaTrajectoryEditingAction> actionList = actions
				.get(currentModification);
		final OmegaTrajectoryEditingAction lastAction = actionList
		...
		List<OmegaTrajectoryEditingAction> cancelledActionList = null;
		if (cancelledActions.containsKey(currentModification)) {
			cancelledActionList = cancelledActions.get(currentModification);
		} else {
			cancelledActionList = new ArrayList<OmegaTrajectoryEditingAction>();
		}
	...
	private void redoLastAction() {
		...
		final List<OmegaTrajectoryEditingAction> cancelledActionList = cancelledActions
				.get(currentModification);
		final OmegaTrajectoryEditingAction lastAction = cancelledActionList
		...
		List<OmegaTrajectoryEditingAction> actionList = null;
		if (actions.containsKey(currentModification)) {
			actionList = actions.get(currentModification);
		} else {
			actionList = new ArrayList<OmegaTrajectoryEditingAction>();
		}
		if (actionList.isEmpty()) {
	...
	private void cancelAllActions() {
		final StringBuffer buf = new StringBuffer();
		buf.append(OmegaTrajectoryEditingPluginConstants.CANCEL_CONFIRM_MSG);

		final GenericConfirmationDialog dialog = new GenericConfirmationDialog(
				this.getParentContainer(), OmegaTrajectoryEditingPluginConstants.CANCEL_CONFIRM,
				buf.toString(), true);
		dialog.setVisible(true);
		if (!dialog.getConfirmation())
			return;
	...
	private void markActionsApplied(final OmegaParticleLinkingRun analysisRun) {
		if (this.actions.containsKey(analysisRun)) {
			final List<OmegaTrajectoryEditingAction> actionList = this.actions
					.get(analysisRun);
			for (final OmegaTrajectoryEditingAction action : actionList) {
	...
	protected void mergeTrajectories(final OmegaTrajectory from,
		...
		final String trajName1 = from.getName() + OmegaTrajectoryEditingPluginConstants.MERGE_APPENDIX
				+ to.getName();
		...
		// newTraj.setName(from.getName() + OmegaTrajectoryEditingPluginConstants.MERGE_APPENDIX
		// + to.getName());
		...
		final OmegaTrajectoryEditingAction action = new OmegaTrajectoryEditingAction(original, modified);
	...
	protected void splitTrajectory(final OmegaTrajectory from,
		...
		final String trajName1 = from.getName() + OmegaTrajectoryEditingPluginConstants.SPLIT_APPENDIX_1;
		final Double index = from.getIndex();
		int tmpIndex = tracks.indexOf(from) + 1;
		int increment = 0;
		if (tmpIndex >= tracks.size()) {
		...
		final OmegaTrajectory newTraj1 = new OmegaTrajectory(particleIndex,
				trajName1, newIndex);
		// newTraj1.setName(from.getName() + OmegaTrajectoryEditingPluginConstants.SPLIT_APPENDIX_1);
		...
		final String trajName2 = from.getName() + OmegaTrajectoryEditingPluginConstants.SPLIT_APPENDIX_2;
		newIndex += addition;
		final OmegaTrajectory newTraj2 = new OmegaTrajectory(newTrajLength,
				trajName2, newIndex);
		// newTraj2.setName(from.getName() + OmegaTrajectoryEditingPluginConstants.SPLIT_APPENDIX_2);
	...
	private void addAction(
			...
			final OmegaTrajectoryEditingAction action) {
		List<OmegaTrajectoryEditingAction> actionList = null;
		final Map<OmegaAnalysisRun, List<OmegaTrajectoryEditingAction>> actions = this.actions;
	...
	private void updateCurrentModifiedTrajectories(
		...
		if (actions.containsKey(analysisRun)) {
			for (final OmegaTrajectoryEditingAction action : actions.get(analysisRun)) {
	...
	private void fireEventSelectionCurrentTrajectoriesRelinkingRun() {
		final OmegaPluginEvent event = new OmegaPluginEventSelectionTrajectoriesRelinkingRun(
				...
				this.currentlyModifiedTrajectories, true);
		this.getPlugin().fireEvent(event);
	}
