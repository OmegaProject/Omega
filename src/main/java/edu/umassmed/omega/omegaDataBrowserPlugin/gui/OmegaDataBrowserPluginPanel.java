...
public class OmegaDataBrowserPluginPanel extends GenericPluginPanel implements
	...
	public void createAndAddWidgets() {
		...
		this.import_btt.setPreferredSize(OmegaGUIConstants.BUTTON_SIZE_LARGE);
		this.import_btt.setSize(OmegaGUIConstants.BUTTON_SIZE_LARGE);
		...
		this.results_btt.setPreferredSize(OmegaGUIConstants.BUTTON_SIZE_LARGE);
		this.results_btt.setSize(OmegaGUIConstants.BUTTON_SIZE_LARGE);
		...
		this.exportLast_btt.setSize(OmegaGUIConstants.BUTTON_SIZE_LARGE);
		this.exportLast_btt.setEnabled(false);
		this.exportAll_btt = new JButton("Export All Results");
		this.exportAll_btt
				.setPreferredSize(OmegaGUIConstants.BUTTON_SIZE_LARGE);
		this.exportAll_btt.setSize(OmegaGUIConstants.BUTTON_SIZE_LARGE);
	...
	protected void fireDataChangedEvent() {
		...
		// TODO to be tested
		if (this.selectedDataElement instanceof OmegaProject) {
			this.fireEventSelectionProject();
		} else if (this.selectedDataElement instanceof OmegaDataset) {
			this.fireEventSelectionDataset();
		} else if (this.selectedDataElement instanceof OmegaImage) {
			this.fireEventSelectionImage();
		} else {
			this.fireEventSelectionOrphanedContainer();
		}
	...
	private <T> void handleDataExporterHelper(
			final OmegaAnalysisRunContainerInterface analysisContainer,
			final Class<T> clazz, final Map<Integer, T> runs,
			final Map<OmegaAnalysisRun, String> paths, int counter,
			String currentPath, final OmegaAnalysisRun selectedRun) {
		if (analysisContainer instanceof OmegaNamedElement) {
			currentPath += ((OmegaNamedElement) analysisContainer).getName()
					+ File.separator;
		}
		for (final OmegaAnalysisRun analysisRun : analysisContainer
				.getAnalysisRuns()) {
			if (clazz.isInstance(analysisRun)
					&& ((selectedRun == null) || selectedRun
							.equals(analysisRun))) {
				runs.put(counter, (T) analysisRun);
				paths.put(analysisRun, currentPath);
				counter++;
			} else {
				this.handleDataExporterHelper(analysisRun, clazz, runs, paths,
						counter, currentPath, selectedRun);
			}
		}
	}
	...
	private void handleDataExporter(final boolean selectLastOnly) {
		...
		if (this.selectedDataElement != null) {
			final Map<Integer, OmegaAnalysisRunContainerInterface> images = new LinkedHashMap<Integer, OmegaAnalysisRunContainerInterface>();
			images.put(0, this.selectedDataElement);
			ote.setImages(images);
		}

		final Map<OmegaAnalysisRun, String> paths = new LinkedHashMap<OmegaAnalysisRun, String>();
		...
		if (!selectLastOnly) {
			...
			this.handleDataExporterHelper(this.selectedDataElement,
					OmegaParticleDetectionRun.class, pDets, paths, 0, "", null);
			...
			final Map<Integer, OmegaParticleDetectionRun> pDets = new LinkedHashMap<Integer, OmegaParticleDetectionRun>();
			this.handleDataExporterHelper(this.selectedDataElement,
					OmegaParticleDetectionRun.class, pDets, paths, 0, "",
					(OmegaParticleDetectionRun) this.selectedDetectionRun);
			// pDets.put(0, (OmegaParticleDetectionRun)
			// this.selectedDetectionRun);
			pDetRuns.put(0, pDets);
			ote.setParticleDetectionRun(pDetRuns);
		}
		
		if (!selectLastOnly) {
			final Map<Integer, Map<Integer, OmegaSNRRun>> snrRuns = new LinkedHashMap<Integer, Map<Integer, OmegaSNRRun>>();
			final Map<Integer, OmegaSNRRun> snrs = new LinkedHashMap<Integer, OmegaSNRRun>();
			this.handleDataExporterHelper(this.selectedDataElement,
					OmegaSNRRun.class, snrs, paths, 0, "", null);
			snrRuns.put(0, snrs);
			ote.setSNRRun(snrRuns);
		} else if (this.selectedSNRRun != null) {
			final Map<Integer, Map<Integer, OmegaSNRRun>> snrRuns = new LinkedHashMap<Integer, Map<Integer, OmegaSNRRun>>();
			final Map<Integer, OmegaSNRRun> snrs = new LinkedHashMap<Integer, OmegaSNRRun>();
			this.handleDataExporterHelper(this.selectedDataElement,
					OmegaSNRRun.class, snrs, paths, 0, "",
					(OmegaSNRRun) this.selectedSNRRun);
			snrRuns.put(0, snrs);
			ote.setSNRRun(snrRuns);
		}
		...
		if (!selectLastOnly) {
			...
			this.handleDataExporterHelper(this.selectedDataElement,
					OmegaParticleLinkingRun.class, pLinks, paths, 0, "", null);
			...
			final Map<Integer, OmegaParticleLinkingRun> pLinks = new LinkedHashMap<Integer, OmegaParticleLinkingRun>();
			this.handleDataExporterHelper(this.selectedDataElement,
					OmegaParticleLinkingRun.class, pLinks, paths, 0, "",
					(OmegaParticleLinkingRun) this.selectedLinkingRun);
			pLinkRuns.put(0, pLinks);
			ote.setParticleLinkingRun(pLinkRuns);
		}
		...
		if (!selectLastOnly) {
			...
			this.handleDataExporterHelper(this.selectedDataElement,
					OmegaTrajectoriesRelinkingRun.class, pRelinks, paths, 0,
					"", null);
			...
			final Map<Integer, Map<Integer, OmegaTrajectoriesRelinkingRun>> pRelinkRuns = new LinkedHashMap<Integer, Map<Integer, OmegaTrajectoriesRelinkingRun>>();
			final Map<Integer, OmegaTrajectoriesRelinkingRun> pRelinks = new LinkedHashMap<Integer, OmegaTrajectoriesRelinkingRun>();
			this.handleDataExporterHelper(
					this.selectedDataElement,
					OmegaTrajectoriesRelinkingRun.class,
					pRelinks,
					paths,
					0,
					"",
		...
		if (!selectLastOnly) {
			...
			this.handleDataExporterHelper(this.selectedDataElement,
					OmegaTrajectoriesSegmentationRun.class, pSegms, paths, 0,
					"", null);
		...
		if (!selectLastOnly) {
			...
			this.handleDataExporterHelper(this.selectedDataElement,
					OmegaTrackingMeasuresIntensityRun.class, intes, paths, 0,
					"", null);
			...
			this.handleDataExporterHelper(this.selectedDataElement,
					OmegaTrackingMeasuresVelocityRun.class, velos, paths, 0,
					"", null);
			...
			this.handleDataExporterHelper(this.selectedDataElement,
					OmegaTrackingMeasuresMobilityRun.class, mobis, paths, 0,
					"", null);
			...
			if (this.selectedTrackingMeasuresRun instanceof OmegaTrackingMeasuresIntensityRun) {
				...
				this.handleDataExporterHelper(
						this.selectedDataElement,
						OmegaTrackingMeasuresVelocityRun.class,
						velos,
						paths,
						0,
						"",
				...
				this.handleDataExporterHelper(
						this.selectedDataElement,
						OmegaTrackingMeasuresMobilityRun.class,
						mobis,
						paths,
						0,
						"",
				...
				this.handleDataExporterHelper(
						this.selectedDataElement,
						OmegaTrackingMeasuresDiffusivityRun.class,
						diffs,
						paths,
						0,
						"",
	...
	private void handleTracksImporter() {
		...
		// TODO need to find a better way to handle this here
		// It's a horrible hack and works only because atm there is only 1
		// segmentation types
		oti.setSegmentationTypes(this.omegaData.getSegmentationTypesList().get(
				0));
	...
	private void fireEventSelectionOrphanedContainer() {
		final OmegaPluginEvent event = new OmegaPluginEventSelectionOrphaned(
				this.getPlugin(),
				(OrphanedAnalysisContainer) this.selectedDataElement);
		this.getPlugin().fireEvent(event);
	}

	private void fireEventSelectionProject() {
		final OmegaPluginEvent event = new OmegaPluginEventSelectionProject(
				this.getPlugin(), (OmegaProject) this.selectedDataElement);
		this.getPlugin().fireEvent(event);
	}

	private void fireEventSelectionDataset() {
		final OmegaPluginEvent event = new OmegaPluginEventSelectionDataset(
				this.getPlugin(), (OmegaDataset) this.selectedDataElement);
		this.getPlugin().fireEvent(event);
	}

	private void fireEventSelectionImage() {
		final OmegaPluginEvent event = new OmegaPluginEventSelectionImage(
				this.getPlugin(), (OmegaImage) this.selectedDataElement);
		this.getPlugin().fireEvent(event);
	}
