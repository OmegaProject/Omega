...
public class OmegaMySqlReader extends OmegaMySqlGateway {
	...
	public OmegaSNRRun loadSNRAnalysis(final long id,
		...
		if (results2 == null)
			return null;
		final Double avgCenterSig = results2
				.getDouble(OmegaMySqlCostants.SNR_AVG_CENTER_SIG);
		final Double avgPeakSig = results2
				.getDouble(OmegaMySqlCostants.SNR_AVG_PEAK_SIG);
		final Double avgMeanSig = results2
				.getDouble(OmegaMySqlCostants.SNR_AVG_MEAN_SIG);
		...
		final OmegaSNRRun snrRun = new OmegaSNRRun(experimenter,
				algorithmSpecification, timeStamps, name,
				...
				resultingLocalErrorIndexSNR, avgCenterSig, avgPeakSig,
				avgMeanSig, background, noise, avgSNR, minSNR, maxSNR,
				avgErrorIndexSNR, minErrorIndexSNR, maxErrorIndexSNR);
	...
	public Map<Long, Double> loadSNRPlaneAvgCenterSignal(final long analysisID)
			throws SQLException {
		return this.loadSNRPlaneDoubleValuesMap(analysisID,
				OmegaMySqlCostants.SNR_IMAGE_AVG_CENTER_TABLE);
	}

	public Map<Long, Double> loadSNRPlaneAvgPeakSignal(final long analysisID)
			throws SQLException {
		return this.loadSNRPlaneDoubleValuesMap(analysisID,
				OmegaMySqlCostants.SNR_IMAGE_AVG_PEAK_TABLE);
	}

	public Map<Long, Double> loadSNRPlaneAvgMeanSignal(final long analysisID)
			throws SQLException {
		return this.loadSNRPlaneDoubleValuesMap(analysisID,
				OmegaMySqlCostants.SNR_IMAGE_AVG_MEAN_TABLE);
	}
	...
	public OmegaTrajectory loadTrajectory(final long id) throws SQLException {
		...
		if (results1 == null)
			return null;
		final String name = results1.getString(OmegaMySqlCostants.NAME_FIELD);
		final Double index = results1.getDouble(OmegaMySqlCostants.INDEX_FIELD);
		...
		results1.getStatement().close();
		results1.close();
		final OmegaTrajectory track = new OmegaTrajectory(length, name, index);
		// track.setName(name);
