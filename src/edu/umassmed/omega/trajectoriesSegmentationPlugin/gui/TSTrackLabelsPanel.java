...
public class TSTrackLabelsPanel extends GenericPanel {
	...
	public TSTrackLabelsPanel(final RootPaneContainer parent,
	        final int orientation, final double realSize,
	        final double pixelsSize, final int panelSize, final int center) {
		super(parent);
	...
	private void paintMetricsFromZero(final Graphics2D g2D) {
		...
		for (int i = 0; i <= numOfLabels; i++) {
			final int coord = new BigDecimal(String.valueOf(i * offsetCoord))
			        .setScale(0, RoundingMode.HALF_UP).intValue();
			final double val1 = new BigDecimal(String.valueOf(offsetVal * i))
