...
public class GenericBrowserModel extends AbstractTableModel {

	private static final long serialVersionUID = -2618857102779599857L;

	private final GenericBrowserTable table;
	private final List<GenericBrowserItem> items;

	public GenericBrowserModel(final GenericBrowserTable table) {
		this.table = table;
		this.items = new ArrayList<GenericBrowserItem>();
		// this.update(elements);
	}

	public GenericBrowserItem getSelectedItem() {
		final int index = this.table.getSelectedRow();
		return this.getItem(index);
	}

	public GenericBrowserItem getItem(final int index) {
	...
	public void update(final Map<Long, String> elements,
		...
		for (final Long id : elements.keySet()) {
			...
			final GenericBrowserItem item = new GenericBrowserItem(id, name);
	...
	public void selectId(final Long id) {
		for (int index = 0; index < this.items.size(); index++) {
			final GenericBrowserItem item = this.items.get(index);
			if (item.getId() == id) {
