...
public class TBFrameImagesLoader implements Runnable {
	...
	public void updateStatus(final String status, final boolean repaint) {
		try {
			SwingUtilities.invokeAndWait(new Runnable() {
				@Override
				public void run() {
					TBFrameImagesLoader.this.displayerPanel
					        .updateMessageStatus(new TBLoaderMessage(status,
					                repaint));
				}
			});
